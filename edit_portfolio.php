...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['upload'])) {
        ...
        if ($conn->query($sql) === TRUE) {
            $uploadSuccessMessage = "Project uploaded successfully!";
        } else {
            $uploadSuccessMessage = "Error: " . $conn->error;
        }
    } elseif (isset($_POST['delete'])) {
        // Handle project deletion
        $projectId = $conn->real_escape_string($_POST['project_id']);
...
$query = "SELECT id, name, description, image FROM projects";
    ...
    <div class="content">
        ...
        <div class="form-section">
            <form action="edit_portfolio.php" method="POST" enctype="multipart/form-data">
                ...
                <input type="submit" name="upload" value="Upload Project" onclick="showUploadAlert('<?= $uploadSuccessMessage ?>');">
            </form>
        </div>
            ...
            <tbody>
                ...
                if (isset($result) && $result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<tr>
                            <td><img src='$imageSrc' alt='{$row['name']}' style='width: 100px; height: 100px;'></td>
                            <td>{$row['name']}</td>
                            <td>{$row['description']}</td>
                            <td>
                                <form action='edit_portfolio.php' method='POST' style='display:inline;' onsubmit=\"showDeleteAlert('Project deleted successfully!');\">
